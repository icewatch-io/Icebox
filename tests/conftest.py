        ...
        while True:
            if not log_file.exists():
                ...
                if time.time() - start_time >= timeout:
                    pytest.fail(
                        f"Timeout: File '{file_path}' not found after {timeout} seconds"
                    )
                continue
            ...
            if time.time() - start_time >= timeout:
                pytest.fail(
                    f"Timeout: Pattern '{pattern}' not found after timestamp {search_time} in '{file_path}' after {timeout} seconds"
                )
